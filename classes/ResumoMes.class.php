<?php
	...
	class ResumoMes extends MainRelatorio
	{
		...
		public function __construct($dados = array()){
			...
			if( $check != 'text/xml' )
			{
				...
				echo "<a href='../".HOSTNAME."/relatorios/novo'>VOLTAR</a>";
				exit;
			}
		...
		public function extrair_dados()
		{
			...
			foreach($consulta as $this->dados)
			{
				...
				$this->total_embalagem_fechada[] = str_replace(',', '.', (string)$this->dados->QTD_ALMN_EMB_FCHD) . " " . (string)$this->dados->COD_TIP_EMB_FCHD . " " . str_replace(',', '.', (string)$this->dados->QTD_CPCD_EMB_FCHD) . " " . (string)$this->dados->NOM_MDD_MATE;
				
				$this->total_embalagem_fracionada[] = str_replace(',', '.', (string)$this->dados->QTD_ALMN_EMB_FRCO) . " " . (string)$this->dados->COD_TIP_EMB_FRCO . " " . str_replace(',', '.', (string)$this->dados->QTD_CPCD_EMB_FRCO) . " " . (string)$this->dados->NOM_MDD_MATE;
